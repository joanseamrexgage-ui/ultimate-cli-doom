...
class RaycastingEngine:
    ...
    def cast_ray(self, angle: float, world_map: List[str]) -> Tuple[float, str]:
        ...
        while True:
            ...
            if world_map[map_y][map_x] != '.':
                distance = math.sqrt(
                    (ray_x - self.player.x)**2 + 
                    (ray_y - self.player.y)**2
                )
    ...
    def get_wall_char(self, wall_type: str, distance: float) -> str:
        ...
        base_char = self.wall_chars.get(wall_type, '█')
    ...
    def render_3d(self, world: World, enemies: List) -> List[str]:
        """
        ...
        for enemy in enemies:
            ...
            if abs(enemy_angle) < self.fov / 2:
                ...
                if 0 <= screen_x < self.width:
                    enemy_char = enemy.get_sprite(distance)
